...
class ComprehensiveTestRunner:
    ...
    def run_command(
        self, cmd: List[str], timeout: int = 300, cwd: Path = None, env: dict = None
    ) -> Dict[str, Any]:
    ...
    def build_tests(self) -> bool:
        ...
        config_result = self.run_command(
            ["cmake", str(self.build_dir.parent)], cwd=self.build_dir
        )
    ...
    def run_framework_tests(self) -> Dict[str, Any]:
        ...
        result = self.run_command(
            [str(self.test_executable), "--gtest_filter=TestFrameworkTest.*"], env=env
        )
    ...
    def run_rom_tests(self, roms: List[str]) -> Dict[str, Any]:
        ...
        result = self.run_command(
            [str(self.test_executable), "--gtest_filter=TestFrameworkWithROMsTest.*"],
            env=env,
        )
    ...
    def run_cpu_tests(self) -> Dict[str, Any]:
        ...
        result = self.run_command(
            [str(self.test_executable), "--gtest_filter=ROMTest.*"], env=env
        )
    ...
    def run_rom_analysis(self, roms: List[str]) -> Dict[str, Any]:
        ...
        for rom in roms:
            ...
            result = self.run_command(["python", "tools/rom_analyzer.py", rom])
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> None:
        ...
        with open(text_report_file, "w") as f:
            ...
            for i, result in enumerate(results, 1):
                f.write(
                    f"Test {i}: {'✅ PASSED' if result.get('success') else '❌ FAILED'}\n"
                )
    ...
    def run_all_cpp_tests(self) -> Dict[str, Any]:
        ...
        env = os.environ.copy()
        env["LLVM_PROFILE_FILE"] = str(self.build_dir / "default.profraw")
        result = self.run_command([str(self.test_executable)], env=env)
        if result["success"]:
