...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Remove all test outputs")
    parser.add_argument(
        "--old", action="store_true", help="Remove old test reports based on config"
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    try:
        if args.all:
            ...
            config = load_config()
            output_dir = Path(
                config.get("reporting", {}).get("output_directory", "test_output")
            )
            max_age = args.days or config.get("reporting", {}).get(
