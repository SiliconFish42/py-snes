...
class ROMAnalyzer:
    ...
    def analyze_header(self):
        ...
        print(
            f"Game Title: {header[0x00:0x15].decode('ascii', errors='ignore').strip()}"
        )
    ...
    def analyze_memory_map(self):
            ...
            if section_type != current_section_type:
                if current_section_start is not None:
                    code_sections.append(
                        (current_section_start, i, current_section_type)
                    )
        ...
        if current_section_start is not None:
            code_sections.append(
                (current_section_start, len(self.rom_data), current_section_type)
            )
